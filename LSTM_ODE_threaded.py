...
class GBPUSDDataModule(pl.LightningDataModule):
    ...
    def load_data(self, train_days, test_days):
        path_to_data = r"/content/gdrive/My Drive/lob_data/" + self.data_type + r"/"
        train_dfs = []
        test_dfs = []
...
class ODELSTMCell(nn.Module):
    ...
    def forward(self, inputs, hx, ts):
        ...
        for batch_idx, batch in enumerate(ts):
            ...
            p = multiprocessing.Process(target=self.trajectory, args=(hx[0][batch_idx], batch, queue, batch_inx))
            processes.append(p)
            p.start()
    ...
    def trajectory(self, hs, dt, queue, batch_idx):
        queue.put((self.ode.trajectory(hs, dt)[1], batch_idx))
...
if __name__ == '__main__':
    ...
    data_module = GBPUSDDataModule(data_type="WTB", window=50, batch_size=64, pred_horizon=20, alpha=0.0000, with_time=True)
    # model = LSTM(input_size=7, hidden_size=100, seq_len=50, num_layers=2)
    model = ODELSTM(input_size=7, hidden_size=100, seq_len=50)
    logger = TensorBoardLogger('tb_logs', name='LSTM_100_seq_len')
