...
class ODE_LSTM_cell(nn.Module):
    def __init__(self, input_size, hidden_size):
        super(ODE_LSTM_cell, self).__init__()
        ...
        self.ode = NeuralDE(f, solver='rk4', sensitivity='autograd')
    
    def forward(self, input, hx, ts):
        new_h, new_c = self.lstm(input, hx)
        new_h = self.ode_solve(new_h, ts)

        return (new_h, new_c)

    def ode_solve(self, x, ts):
        ts = ts.view(-1, 1)
        for i in range(3):
            x = self.ode(x, ts *(1.0/3))
        return x
        


class ODE_LSTM(pl.LightningModule):
    def __init__(self, input_size=10, hidden_size=100, num_layers=2, output_size=1, batch_size=1):
        super().__init__()
        self.lstm_cell = ODE_LSTM_cell(input_size=input_size, hidden_size=hidden_size)
    ...
    def forward(self, x, timespans):
        lstm_out, self.hidden = self.lstm_cell(x.view(len(x), 1, -1), self.hidden_cell)
